...
//             //             .start()
//             //             .awaitTermination
//  val res = df.writeStream.outputMode("append").partitionBy("year","Month").format("parquet").option("checkpointLocation","/checkpointfe/click").option("path","/warehouse/click").start
//                         .start()
//                         .awaitTermination
// }
